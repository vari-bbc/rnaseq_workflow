...
```{r leading, fig.height=20, fig.width=7}
...
lapply(names(gsea_res), function(geneseq_collec){
  top_geneset_ids <- head(as.data.frame(gsea_res[[geneseq_collec]]$`GenotypeAKO-GenotypeWT`$entrez)$ID, 3) # using head instead of indexing accounts for cases where there are less than 'n' significant genesets
  lapply(top_geneset_ids, function(geneset_id){
    plot_leading_edge_heatmap(cpms=cpms, gsea_res=gsea_res, geneset_collection=geneseq_collec, geneset_id=geneset_id)
  })
})
```
