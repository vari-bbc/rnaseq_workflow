...
if (ncol(colData(se)) > 5 & params$numeric_variables != "" ){
...
df %>% 
  dplyr::filter(Symbol %in% params$genes_of_interest) %>% 
...
if (!(tolower(params$group_reg_formula) %in% c("nan", ""))){
  param_list <- unlist(str_split(gsub("~", "", gsub(" ", "", params$group_reg_formula)), "[+]"))
} else{ param_list <- list("group")}
