  ...
  # lapply(setNames(nm=names(gsea_res[[geneset]])), function(contrast){
    ...
    if(nrow(gseaResult) > 0){
      dotplot(gseaResult, split=".sign") + ggtitle(paste0(params$comparison_name," -- ", geneset)) +
        scale_y_discrete(label=function(x) str_wrap(str_trunc(str_remove(x, paste0("HALLMARK_|\\s-\\s", params$msigdb_organism,".*")), 15), width = 60)) + facet_grid(.~.sign)
...
We can use a package called aPEAR to display genes inside networks found in significant pathways from your data. Each ellipse defines a significant pathway and the association of genes within it. 
