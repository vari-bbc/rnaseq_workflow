...
pathwayNames = c("Reactome","BP", "BP-simplified", "KEGG","H","C3","C6","C7")
  ...
  # lapply(setNames(nm=names(gsea_res[[geneset]])), function(contrast){
    ...
    if(nrow(gseaResult) > 0){
      dotplot(gseaResult, split=".sign") + ggtitle(paste0(params$comparison_name," -- ", geneset)) +
        scale_y_discrete(label=function(x) str_trunc(str_remove(x, "HALLMARK_|\\s-\\sRattus norvegicus.*"), 40)) + facet_grid(.~.sign)
    } else{
