...
prep_clusterprofiler_genelist <- function(dge_table, rank_by="signedPvalues", pval_col="pvalue", lfc_col="log2FoldChange"){
  geneList <- NULL

  # calculate rank_metric
  if(identical(rank_by, "signedPvalues")){
    ...
    # Remove zeroes for log transformation; raise to the minimum value the machine is capable of maintaining
    dge_table[[pval_col]] <- dge_table[[pval_col]] + .Machine$double.eps
...
```{r calc_ranks}
genes_and_score <- lapply(de_res, prep_clusterprofiler_genelist, rank_by="signedPvalues")
...
```{r dotplots, fig.width=9, fig.height=5}
dotplots <- lapply(setNames(nm=names(gsea_res)), function(geneset){
  # lapply(setNames(nm=names(gsea_res[[geneset]])), function(contrast){
    ...
    if(nrow(gseaResult) > 0){
      dotplot(gseaResult, split=".sign", showCategory = 5) + ggtitle(paste0(params$comparison_name," -- ", geneset)) +
        scale_y_discrete(label=function(x) str_replace_all(str_trunc(str_remove(x, paste0("HALLMARK_|\\s-\\s", params$msigdb_organism,".*")), 50), "_", " ")) + 
        facet_grid(.~.sign) #+
...
aPEARobj = lapply(seq_along(gsea_res), FUN = function(x){
  ...
  tryCatch({
    ...
    p <- aPEAR::enrichmentNetwork(gseaResObj@result, drawEllipses = TRUE, fontSize = 2.5, minClusterSize=3) + ggtitle(paste0(params$comparison_name," -- ", resSetName))
