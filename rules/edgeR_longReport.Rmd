...
cpm.filter = 1/min(lib_sizes$millions_of_counts) # feature cpm threshold
cat('Derived the cpm filter from a minimum of 1 cpm in the smallest library\n')
...
contrast_list = read.delim(paste0("../",config$contrasts),stringsAsFactors = T ,sep='\t')
...
for (i in 1:length(contrast_list$contrast)){
  ...
  if (nrow(GO.BP@result %>% dplyr::filter(p.adjust < 0.05)) > 0){
  ...
  if (nrow(GO.MF@result %>% dplyr::filter(p.adjust < 0.05)) > 0){
  ...
  if (nrow(GO.CC@result %>% dplyr::filter(p.adjust < 0.05)) > 0){
  ...
  # REACTOME
    ...
    # run fGSEA
    if(any(is.na(geneRanks))){geneRanks=geneRanks[-which(is.na(geneRanks))]} # remove any NAs which breaks clusterProfiler::GSEA
