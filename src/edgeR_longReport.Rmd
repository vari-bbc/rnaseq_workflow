...
if (!exists(c("t2g_no_dup_genes","t2g"))){
  t2g_file='hsapiens_gene_ensembl_go.txt'
  if(t2g_file %in% list.files()){
    t2g = read.delim(t2g_file,sep='\t',stringsAsFactors = FALSE)
  }else{
    mart <- biomaRt::useMart(biomart = 'ENSEMBL_MART_ENSEMBL', host="www.ensembl.org", dataset = 'hsapiens_gene_ensembl', ensemblRedirect = FALSE)
    t2g <- biomaRt::getBM(attributes = c("ensembl_gene_id", "external_gene_name", "description","chromosome_name","start_position","end_position","strand","transcript_length","go_id"), mart = mart) # "kegg_enzyme"
    # dim(t2g)
    t2g <- dplyr::rename(t2g, ens_gene = ensembl_gene_id, ext_gene = external_gene_name, predicted_function = description)
    write.table(t2g,t2g_file,sep='\t',quote=FALSE,row.names = FALSE)
  }
...
if (!file.exists(c("../r_objects/reactome.mapper.RDS","../r_objects/reactome.mapper.RDS"))){
...
reactome.mapper <- readRDS("../r_objects/reactome.mapper.RDS")
reactome.mapper.goseq <- readRDS("../r_objects/reactome.mapper.goseq.RDS")
...
# Filter 1
# 
sample.cpm.filter=7 # How many samples must meet the cpm threshhold
cpm.filter=10/min(lib_sizes$millions_of_counts) # feature cpm threshold
...
```{r DGE_analysis_individual, results='asis'}

contrast_list = read.delim("../contrasts.tsv",stringsAsFactors = F,sep='\t')
...
```{r DGE_analysis_group, results='asis'}

design = model.matrix(~0 + condition, data = meta)

# Loop Through all listed contrasts and run edgeR, GO, GSEAoutput
  myargs = list(paste0("Normal_vs_GBM"," = ",
                       "conditionGBM", " - ",
                       "conditionNormal"),
                       levels=design)
  
  #set values for invocation of edgeR with RNAseq()
  contrast.name = "Normal_vs_GBM"
