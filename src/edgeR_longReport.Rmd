...
raw.data.file <- '../deliverables/counts_.tsv'
...
# PerMANOVA - partitioning the euclidean distance matrix by 'condition'
  # using all PCs
pca_sig <- adonis((pr_comps %>% dplyr::select(-c(sample,condition))) ~ condition,
       data = pr_comps,
       permutations = 9999,
       method='eu')
pca_sig
...
# REACTOME
  ...
  if (nrow(gsea_res@result) >=1){
    ...
    print(ridgeplot(gsea_res) +
            ...
            scale_y_discrete(label=function(x) paste0(abbreviate(x, minlength=40),"...")) +
    ...
    # Full Table
    print(knitr::kable(gsea_res@result, caption='Enriched Reactome Pathways:') %>%
            kable_styling(bootstrap_options = c("striped", "hover","condensed"),font_size = 12))
    }
  }else{
...
# KEGG
  ...
  if (nrow(gsea_res@result) >=1){
    ...
    for (i in 1:min(30,length(gsea_res$Description))){
      print(gseaplot2(gsea_res, geneSetID = i, title = gsea_res$Description[i]))
    # Full Table
    print(knitr::kable(gsea_res@result, caption='Enriched KEGG Pathways:') %>%
            kable_styling(bootstrap_options = c("striped", "hover","condensed"),font_size = 12))
    }
  }else{
