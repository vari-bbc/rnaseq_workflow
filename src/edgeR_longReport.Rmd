...
mapstat_sample <- mapstat_unit %>% 
  group_by(sample) %>%
  summarise(sumUnique=sum(UniquelyMappingReads),sumTotalReads=sum(TotalReads)) %>%
...
lib_sizes <- lib_sizes %>% left_join(meta %>% dplyr::select(sample,condition),by="sample")
...
ggplot(lib_sizes, aes(x = sample, y = millions_of_counts, fill=condition)) +
...
# PCA
# Open first four columns of iris data and scales the entries(rows)
iris_c <- scale(iris[ ,1:4])
# rda calls PCA on the data
pca <- rda(iris_c)
...
# label the centroid of each group
ordispider(pca, groups = iris$Species, label = TRUE)

# PerMANOVA - partitioning the euclidean distance matrix by 'condition'
pr_comps_c <- scale(t(y.voom$E))

#using scaled-normalized expression values
adonis(scale(t(y.voom$E)) ~ condition,
       data = pr_comps,
       permutations = 999,
       method='eu')
#using PC1/PC2
adonis((pr_comps %>% dplyr::select(c(PC1,PC2))) ~ condition,
       data = pr_comps,
       permutations = 999,
       method='eu')
#using all PCs
adonis((pr_comps %>% dplyr::select(-c(sample,condition))) ~ condition,
       data = pr_comps,
       permutations = 999,
       method='eu')
```
...
for (i in 1:length(contrast_list$contrast)){
  ...
  gsea_res <- clusterProfiler::GSEA(geneList, nPerm= 10000, TERM2GENE = m_t2g, by="fgsea")
