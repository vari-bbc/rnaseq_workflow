...
if (!exists(c("t2g_no_dup_genes","t2g"))){
  ...
  if(t2g_file %in% list.files()){
    ...
    mart <- biomaRt::useMart(biomart = 'ENSEMBL_MART_ENSEMBL', host="www.ensembl.org", dataset = 'mmusculus_gene_ensembl', ensemblRedirect = NULL)
...
reactome.mapper = dplyr::filter(reactome.mapper,Species=='Mus musculus')
...
units <- read.delim("../src/units.tsv",sep='\t',header=TRUE,stringsAsFactors = F) %>% 
  ...
  colnames(mapstat)=c('sample_unit','UniquelyMappingRate')
  ...
  mapstat = dplyr::left_join(mapstat,units, by="sample_unit")
  mapstat$sample=factor(mapstat$sample_unit,levels=units$sample_unit)
...
colnames(mapstat)=c('sample_unit','UniquelyMappingReads')
mapstat$sample=gsub(".*/","",mapstat$sample)
mapstat = dplyr::left_join(mapstat,units, by="sample_unit")
mapstat$sample=factor(mapstat$sample_unit,levels=meta$sample_unit)

# ggplot(mapstat,aes(UniquelyMappingReads)) + geom_histogram() #+ facet_grid(genotype~.)

ggplot(mapstat,aes(x=sample_unit,y=UniquelyMappingReads)) + 
...
for (i in 1:length(contrast_list$contrast)){
  ...
  cat("GSEA testing was performed with ClusterProfiler's `enrichGO` function. \n\n")
  ...
  d <- list$table %>% dplyr::select(c(ens_gene,ext_gene,logFC))
  geneList <- d[,3]
  ...
  print(dotplot(GO.BP, showCategory=30) + ggtitle("DotPlot - GO:Biological Process"))
  print(emapplot(GO.BP) + ggtitle("EnrichmentMap - GO:Biological Process"))
  #print(cnetplot(GO.BP, foldChange=geneList) + ggtitle("GO:Biological Process"))
  
  cat("\n\n##### GO: Molecular Function \n\n")
  print(dotplot(GO.MF, showCategory=30) + ggtitle("DotPlot - GO:Molecular Function"))
  print(emapplot(GO.MF) + ggtitle("EnrichmentMap - GO:Molecular Function"))
  #cnetplot(GO.MF, foldChange=geneList) + ggtitle("GO:Molecular Function")
  
  cat("\n\n##### GO: Cellular Compartment \n\n")
  print(dotplot(GO.CC, showCategory=30) + ggtitle("DotPlot - GO:Cellular Compartment"))
  print(emapplot(GO.CC) + ggtitle("EnrichmentMap - GO:Molecular Function"))
  #cnetplot(GO.CC, foldChange=geneList) + ggtitle("GO:Molecular Function")
  
  ## GSEA: Reactome and KEGG ---------------------------------
  cat("\n\n#### GSEA \n\n")  
  
  cat("GSEA testing was performed with ClusterProfiler's `GSEA` function. \n\n")
  ...
  gsea_res <- GSEA(geneList, nPerm= 10000, TERM2GENE = m_t2g, by="fgsea")
  ...
  # KEGG
  cat("\n\n##### KEGG Pathways \n\n")
  m_t2g <- msigdbr(species = "Mus musculus", category = "C2", subcategory = "CP:KEGG") %>% 
    dplyr::select(gs_name, gene_symbol)
  ...
  print("Significantly differentially expressed genes are k-means clustered (k=6).")
  ...
  pheat_enrichGO <- compareCluster(Entrez~clust, data=pheat_clust, fun="enrichGO", OrgDb='org.Mm.eg.db')
  # plot the top5 GO categories for each cluster
  dotplot(pheat_enrichGO, x=~clust, title="GO enrichment by expression cluster")
  
} 
```

```{r seriesClustering_dev}

  # ## do the clustering
  # max_itr <-  50
  # n_clust  <-  4 # number of cluster 
  # set.seed(123) # reproduce the cluster 
  # kmeans_out  <- kmeans(forClust, n_clust, iter.max = max_itr)
  # 
  # ## add cluster info to original matrix 
  # forClust <- forClust %>% 
  #   rownames_to_column(var='ens_gene') %>% 
  #   mutate(clust = kmeans_out$cluster) %>%
  #   dplyr::left_join((t2g_no_dup_genes %>% dplyr::select(ens_gene,ext_gene)),by="ens_gene")
  # 
  # ## visualise each cluster 
  # forClust %>% 
  #   gather(key = "Sample" , value = "Normalized_Expression", -c("ens_gene","ext_gene","clust")) %>%
  #   group_by(Sample) %>%
  #   mutate(row_num =  1:n()) %>%
  #   ggplot(aes(x =  Sample , y = Normalized_Expression , group = row_num)) +
  #   geom_point() +
